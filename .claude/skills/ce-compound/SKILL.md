...
description: |
  Documents a recently solved bug, refactor, or architectural decision into
  docs/solutions/ while context is fresh. Typically invoked by the
  compound-solutions agent (write mode). Direct invocation also supported
  via /ce-compound [context] — compound-solutions handles auto-detection of
  "that worked", "이제 됐어" trigger phrases.
...
#### 4. **Session History via `ce-sessions`** (synchronous skill call, after launching the parallel block — only if the user opted in)
   ...
   - Invoke the `ce-sessions` skill via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi). Pass the dispatch payload below as the skill argument string. `ce-sessions` runs in main context — it owns discovery, branch/keyword filtering, scan-window selection, the deep-dive cap, per-session extraction to a `mktemp` scratch dir, and dispatch of the synthesis-only `session-compounder` subagent. The compound orchestrator only needs to pass the topic and time window and read back the findings text.
...
8. **Run `python3 .claude/skills/ce-compound/scripts/validate-frontmatter.py <output-path>`** to catch silent-corruption parser-safety issues that the prose rules miss: malformed `---` delimiter lines, unquoted ` #` in scalar values (silent comment truncation), and unquoted `: ` in scalar values (silent mapping confusion). Exit 0 means the doc is parser-safe; exit 1 means the script's stderr names the offending field(s) and what to fix — quote the value(s), re-write the doc, and re-run until exit 0. Do not declare success while validation fails. The script does not enforce schema rules and does not flag YAML reserved-indicator characters (those produce loud parser errors downstream rather than silent corruption — out of scope). Uses Python 3 stdlib only (no PyYAML or other deps).
...
Skipped in this extracted cluster. The original plugin's specialized reviewer
agents (ce-performance-oracle, ce-security-sentinel, ce-data-integrity-guardian,
ce-code-simplicity-reviewer, ce-best-practices-researcher,
ce-framework-docs-researcher, ce-pattern-recognition-specialist) are NOT
included. If a domain-specific review is needed, dispatch a generic specialist
via the Agent tool with appropriate role instructions.
...
## Auto-Invoke (DISABLED in this cluster)

Auto-invocation has been removed from this extracted version. All invocations
of this skill must go through the `compound-solutions` agent. Direct
trigger phrases ("that worked", "it's fixed", etc.) no longer fire this
skill — instead they should be picked up by the main flow's CLAUDE.md
routing, which dispatches `compound-solutions`.
