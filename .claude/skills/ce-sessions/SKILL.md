...
- When the answer is already available in the current conversation history
- When searching for content in files rather than agent session logs — use Grep or Read instead
- When no prior agent sessions exist for this repo (new project)
...
```bash
bash .claude/skills/ce-sessions/scripts/discover-sessions.sh <repo> <days> | tr '\n' '\0' | xargs -0 python3 .claude/skills/ce-sessions/scripts/extract-metadata.py --cwd-filter <repo>
```
...
```bash
python3 .claude/skills/ce-sessions/scripts/extract-skeleton.py --output "$SCRATCH/<session-id>.skeleton.txt" < <session-file>
```
...
```bash
python3 .claude/skills/ce-sessions/scripts/extract-skeleton.py --output "$SCRATCH/<session-id>.skeleton.tail.txt" < <session-file>
```
...
```bash
python3 .claude/skills/ce-sessions/scripts/extract-errors.py --output "$SCRATCH/<session-id>.errors.txt" < <session-file>
```
